...
def  main():
  ...
  installer = "\installer\AnyFM Installer.exe"
   
  if demo:
    installer = "\installer\AnyFM Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\AnyFM manual.pdf" 
  ]
  ...
  ZIPNAME = "AnyFM-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "AnyFM-v" + FULLVERSIONSTR + "-win-demo.zip"
